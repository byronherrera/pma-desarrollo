...
function selectProcedimientosCadena($procLista)
{
    ...
    if (isset($procLista)) {
        ...
        $sql = "SELECT amc_entidades.nombre FROM amc_entidades WHERE id in ( $procLista ) ORDER BY id";
...
function selectTiposOperativos()
{
    global $os;
    ...
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT * FROM amc_entidades $where $orderby LIMIT $start, $limit";
    $result = $os->db->conn->query($sql);
    $data = array();
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
        $data[] = $row;
    ...
    $sql = "SELECT count(*) AS total FROM amc_entidades $where";
    $result = $os->db->conn->query($sql);
...
function insertTiposOperativos()
{
    global $os;
    ...
    $sql = "INSERT INTO amc_entidades($cadenaCampos)
	values($cadenaDatos);";
...
function generaCodigoProcesoOrdenanza()
{
    ...
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT MAX(id) AS maximo FROM amc_entidades";
    $result = $os->db->conn->query($sql);
    $row = $result->fetch(PDO::FETCH_ASSOC);
    if (isset($row['maximo'])) {
...
function updateTiposOperativos()
{
    global $os;
    $os->db->conn->query("SET NAMES 'utf8'");
    ...
    $sql = "UPDATE amc_entidades SET  $cadenaDatos  WHERE amc_entidades.id = '$data->id' ";
    $sql = $os->db->conn->prepare($sql);
    $sql->execute();

    echo json_encode(array(
        "success" => $sql->errorCode() == 0,
        "msg" => $sql->errorCode() == 0 ? "Ubicación en amc_entidades actualizado exitosamente" : $sql->errorCode(),
        "message" => $message
    ));
}
...
function validarCedulaCorreo($id)
{
    ...
    global $os;
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT cedula, email FROM amc_entidades WHERE id = $id";
    $result = $os->db->conn->query($sql);
...
function selectOrdenanzasForm()
{
    ...
    $id = (int)$_POST ['id'];
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT *, (SELECT numero FROM amc_guias WHERE amc_guias.id = a.guia ) as guianumero, (SELECT COUNT(*) FROM amc_entidades  b WHERE a.cedula = b.cedula and b.cedula <> '') as totaldocumentos FROM amc_entidades as a  WHERE a.id = $id";
    $result = $os->db->conn->query($sql);
    $data = array();
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
...
function deleteTiposOperativos()
{
    global $os;
    $id = json_decode(stripslashes($_POST["data"]));
    $sql = "DELETE FROM amc_entidades WHERE id = $id";
    $sql = $os->db->conn->prepare($sql);
    $sql->execute();
    echo json_encode(array(
        "success" => $sql->errorCode() == 0,
        "msg" => $sql->errorCode() == 0 ? "Ubicación en amc_operativos_tipos, eliminado exitosamente" : $sql->errorCode()
    ));
}

switch ($_GET['operation']) {
    case 'select' :
        selectTiposOperativos();
        break;
    case 'insert' :
        insertTiposOperativos();
        break;
    case 'update' :
        updateTiposOperativos();
        break;
    case 'selectForm' :
