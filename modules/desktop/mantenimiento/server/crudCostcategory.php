...
function selectOrdenanzas()
{
    ...
    $sql = "SELECT count(*) AS total FROM pma_cost_category $where";
    $result = $os->db->conn->query($sql);
...
function insertOrdenanzas()
{
    ...
    $sql = "INSERT INTO pma_cost_category($cadenaCampos)
	values($cadenaDatos);";
...
function generaCodigoProcesoOrdenanza()
{
    ...
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT MAX(id) AS maximo FROM pma_cost_category";
    $result = $os->db->conn->query($sql);
    $row = $result->fetch(PDO::FETCH_ASSOC);
    if (isset($row['maximo'])) {
...
function updateOrdenanzas()
{
    ...
    $sql = "UPDATE pma_cost_category SET  $cadenaDatos  WHERE pma_cost_category.id = '$data->id' ";
    $sql = $os->db->conn->prepare($sql);
    $sql->execute();

    echo json_encode(array(
        "success" => $sql->errorCode() == 0,
        "msg" => $sql->errorCode() == 0 ? "Ubicación en pma_cost_category actualizado exitosamente" : $sql->errorCode(),
        "message" => $message
    ));
}
...
function validarCedulaCorreo($id)
{
    ...
    global $os;
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT cedula, email FROM pma_cost_category WHERE id = $id";
    $result = $os->db->conn->query($sql);
...
function selectOrdenanzasForm()
{
    ...
    $id = (int)$_POST ['id'];
    $os->db->conn->query("SET NAMES 'utf8'");
    $sql = "SELECT *, (SELECT numero FROM amc_guias WHERE amc_guias.id = a.guia ) as guianumero, (SELECT COUNT(*) FROM pma_cost_category  b WHERE a.cedula = b.cedula and b.cedula <> '') as totaldocumentos FROM pma_cost_category as a  WHERE a.id = $id";
    $result = $os->db->conn->query($sql);
    $data = array();
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
...
function updateOrdenanzasForm()
{
    ...
    $sql = "UPDATE pma_cost_category SET 
            id = '$id',
            nombre = $nombre,
...
function deleteOrdenanzas()
{
    ...
    $sql = "DELETE FROM pma_cost_category WHERE id = $id";
    $sql = $os->db->conn->prepare($sql);
    $sql->execute();
    echo json_encode(array(
        "success" => $sql->errorCode() == 0,
        "msg" => $sql->errorCode() == 0 ? "Ubicación en pma_cost_category, eliminado exitosamente" : $sql->errorCode()
    ));
}
