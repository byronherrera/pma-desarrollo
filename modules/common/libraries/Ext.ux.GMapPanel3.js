...
Ext.ux.GMapPanel = Ext.extend(Ext.Panel, {
    ...
    geoCodeLookup2: function (addr, marker, clear, center, listeners) {
        if (!this.geocoder) {
            this.geocoder = new google.maps.Geocoder();
        }
        this.geocoder.geocode({
            address: addr
        }, this.addAddressToMap2.createDelegate(this, [addr, marker, clear, center, listeners], true));
    },
    ...
    // private
    // private
    addAddressToMap2: function (response, status, addr, marker, clear, center, listeners) {
        if (!response || status !== 'OK') {
            this.respErrorMsg(status);
        } else {
            var place = response[0].geometry.location;
